...
module.exports = {
	...
	apidoc: {
		// The prefix for the OpenAPI documentation. Documentation is always
		// available from '{prefix}/swagger.json'
		prefix: '/apidoc',

		// Overrides for the OpenAPI documentation. These optional
		// overrides tweak how the API is generated, but not how it is hosted
		// (use `apiPrefix`, `port`, and `ssl` configuration instead). This
		// allows you to tweak specific OpenAPI values that are useful when
		// the service is not consumed directly, such as when the services is
		// exposed through a proxy. Available options are:
		//
		// schemes - OpenAPI 2 specific override. The transfer protocol of
		// the service. It is an array of values that must be one or more of
		// 'http', 'https', 'ws' or 'wss'.
		// e.g. ['https']. It can be null to specify not to use schemes.
		//
		// host - OpenAPI 2 specific override. The host (name or ip) serving
		// the service. This MUST be the host only and does not include
		// the scheme nor sub-paths. It may include a port. It can be null to
		// specify not to use host.
		//
		// basePath - OpenAPI 2 specific override. The base path on which
		// the service is served relative to the host. If provided, this MUST
		// start with a leading slash, or be null to specify not to use basePath.
		//
		// servers - OpenAPI 3 specific override. If provided, this MUST be
		// an array or null to specify not to use servers.
		overrides: {
			// schemes: [ 'https' ],
			// host: 'localhost:8080',
			// basePath: '/',
	...
	session: {
		...
		encryptionKey: 'qwzQYC+OYUMxnlCA+SdWVibAC6+uVVDEBuplFq31U/k=',
		signatureAlgorithm: 'sha512-drop256',
		signatureKey: 'hsY6UT/lKvC9kZTqxwTXbzxC0ZpRKAws4EBnUCqroNfNQVP679pVFzbKeQdDNpFZqkvoedD6TWao24luPFcgjg==',
		// should be a large unguessable string
		secret: 'Og87fbIqFf8nDaqtjK1WWUcj9aFNsQll',
	...
	cookieSecret: '/+fLb3jgP+5kP+lhJoZ5AlWNy6Do8r+p',
	...
	flags: {
		...
		// Enabling this flag will exclude legacy HTTP headers "server", "etag",
		// and "content-md5" from client responses by default (unless otherwise
		// configured in `http.headers`).
